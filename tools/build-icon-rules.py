...
doc = {
  ...
  "how_to_query": {
    ...
    "before_any_of_that": "Ask whether it should be an icon at all. See docs/principles.md "
                          "principle 1 — a label is often clearer, and icons added for their "
                          "own sake are noise.",
    "localising": "See cultural_semantics. It is a hazard list, NOT a cleared list.",
    "in_a_product": "See product_binding. A product manifest outranks a general "
                    "recommendation here."
  },
  "coverage": {
  ...
  "cultural_semantics": {
    "status": "OPEN — this is a hazard list, not an audit. No locale review has been done. "
              "Treat any icon encoding correctness, direction, a hand, or a physical object "
              "as UNVERIFIED outside a Western context.",
    "rule": "A mark does not mean the same thing everywhere. Check before assuming universal.",
    "known_hazards": [
      {
        "id": "maru-batsu",
        "icons": ["circle", "circle-fill", "check", "check-circle", "close", "close-circle"],
        "locales": ["ja", "ko", "zh-TW"],
        "issue": "In Japan, and broadly across Korea and Taiwan, MARU (a circle) means correct "
                 "or yes and BATSU (a cross) means wrong or no. Western products use a check "
                 "for correct and treat a circle as neutral — a radio, a status dot, an empty "
                 "state.",
        "consequence": "A circle used as a neutral or empty marker can read as an assertion "
                       "of correctness; a bare cross used to dismiss can read as marking "
                       "something wrong rather than closing it.",
        "action": "Unresolved. Do not assume the Western reading holds."
      },
      {
        "id": "rtl-mirroring",
        "icons": ["arrow-*", "chevron-*", "caret-*", "arrow-turn-*", "navigation",
                  "log-out", "share", "external", "skip-forward", "skip-back",
                  "sort-order", "swap", "route", "car", "rv", "trailer"],
        "locales": ["ar", "he", "fa", "ur"],
        "issue": "53 icons in this set point somewhere. In right-to-left locales the reading "
                 "axis reverses, so an unmirrored 'next' arrow means backwards.",
        "consequence": "Directional affordances invert silently. Nothing errors; users just "
                       "go the wrong way.",
        "action": "Unresolved, and the largest single i18n item. Needs a per-icon call: "
                  "directional and flow icons mirror; icons of physical objects with a fixed "
                  "orientation usually do not, and media transport controls are a documented "
                  "argument in both directions."
      },
      {
        "id": "local-objects",
        "icons": ["mail", "money", "price", "parking", "ev-charger", "fuel"],
        "issue": "A flag-on-a-post mailbox is North American. A dollar sign is not currency. "
                 "Parking, plugs and pumps look different by region.",
        "action": "Unresolved. Prefer the most abstract form that still reads."
      },
      {
        "id": "hands-and-gestures",
        "icons": [],
        "issue": "A thumb, an OK sign, a pointing finger — each is rude somewhere.",
        "action": "None in the set today. Keep it that way unless there is a strong reason, "
                  "and treat any proposal for one as needing a locale review first."
      }
    ]
  },
  "product_binding": {
    "rule": "The library says what an icon IS. A product manifest says what it DOES HERE. "
            "One icon may carry several meanings and several icons may be defensible for one "
            "action — that is resolved per product, not by deleting drawings from the library.",
    "precedence": "A product binding outranks any general recommendation in `icons`. If they "
                  "disagree, the product wins and the disagreement is worth recording.",
    "shape": {
      "product": "roadtrip",
      "bindings": [
        {"action": "<what the user does>", "icon": "<icon name>",
         "replaces": "<what the product ships today, if anything>",
         "note": "<why this one and not the other candidate>"}
      ]
    },
    "why_a_manifest": "Written down once, it stops being a debate that gets re-had in every "
                      "review — and it is the only place the ambiguity CAN be resolved, "
                      "because the answer depends on the product's own flows.",
    "example": {
      "product": "roadtrip",
      "status": "SEED — drawn from the fourteen emoji and eleven text glyphs the app ships as "
                "controls today (docs/proposal-travel-outdoors-pack.md). Not ratified, and not "
                "complete: it covers what the audit actually found, not the whole product.",
      "bindings": [
        {"action": "delete a saved trip", "icon": "trash", "replaces": "\U0001F5D1",
         "note": "destroys the record — not `close-circle`, which only dismisses"},
        {"action": "edit trip details", "icon": "edit", "replaces": "\u270f\ufe0f"},
        {"action": "open settings", "icon": "settings", "replaces": "\u2699"},
        {"action": "start playback / begin", "icon": "play", "replaces": "\u25b6"},
        {"action": "alerts", "icon": "bell", "replaces": "\U0001F514"},
        {"action": "warn about a problem", "icon": "warning-fill", "replaces": "\u26a0",
         "note": "FILLED — it has to be read before the sentence beside it"},
        {"action": "confirm success", "icon": "check-circle-fill", "replaces": "\u2705",
         "note": "filled, same reason"},
        {"action": "add to cart", "icon": "cart-add", "replaces": "\U0001F6D2"},
        {"action": "a campsite", "icon": "campsite", "replaces": "\U0001F3D5\ufe0f",
         "note": "one reservable pitch; use `campground` for the whole facility"},
        {"action": "terrain / scenery", "icon": "mountain", "replaces": "\u26f0\ufe0f"},
        {"action": "a reservation", "icon": "price", "replaces": "\U0001F39F\ufe0f",
         "note": "no `ticket` icon exists and none is proposed — `price` covers it"},
        {"action": "view / preview", "icon": "eye", "replaces": "\U0001F440"},
        {"action": "retry or repeat", "icon": "refresh", "replaces": "\U0001F501"},
        {"action": "live status dot", "icon": "circle-fill", "replaces": "\U0001F7E2",
         "note": "SEE cultural_semantics.maru-batsu before shipping this outside en-*"},
        {"action": "open the nav drawer", "icon": "menu", "replaces": "\u2630"},
        {"action": "close a modal", "icon": "close", "replaces": "\u2715 / \u00d7"},
        {"action": "favourite", "icon": "star", "replaces": "\u2605 / \u2606",
         "note": "star, not `heart` — roadtrip ranks these, it does not react to them"},
        {"action": "sort ascending / descending", "icon": "caret-up", "replaces": "\u25b2 / \u25bc"},
        {"action": "expand a row", "icon": "chevron-down", "replaces": "\u25be"},
        {"action": "drill into a row", "icon": "chevron-right", "replaces": "\u25b8",
         "note": "MIRRORS in RTL — see cultural_semantics.rtl-mirroring"},
        {"action": "directions to a place", "icon": "navigation",
         "replaces": "a trending-up arrow",
         "note": "the app misuses a trend arrow for this today"}
      ]
    }
  },
  "clusters": CLUSTERS,
